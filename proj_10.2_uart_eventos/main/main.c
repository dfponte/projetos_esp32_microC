...
void uart_event_task(void *pvParameters)
{
    ...
    while(1) 
    {
        ...
        if(xQueueReceive(uart0_queue, (void * )&event, (TickType_t)portMAX_DELAY))  // Wait for UART event
        {
            ...
            ESP_LOGI(TAG, "uart[%d] event:", UART_NUM_1);       // Print UART event
            switch(event.type) 
            {
                ...
                case UART_DATA:                                                         // Data event
                    ...
                    uart_read_bytes(UART_NUM_1, dtmp, event.size, portMAX_DELAY);       // Read UART data
                    ESP_LOGI(TAG, "[DATA EVT]: %s",dtmp);                                       // Print UART data
                    uart_write_bytes(UART_NUM_1, (const char*) dtmp, event.size);       // Write UART data
                    break;
                //Event of HW FIFO overflow detected
                case UART_FIFO_OVF:                                                      // FIFO overflow event
                    ESP_LOGI(TAG, "hw fifo overflow");                                   // Print FIFO overflow event               
                    uart_flush_input(UART_NUM_1);                                       // Flush UART buffer
                ...
                case UART_PATTERN_DET:
                    uart_get_buffered_data_len(UART_NUM_1, &buffered_size);             // Get UART buffer size
                    int pos = uart_pattern_pop_pos(UART_NUM_1);                         // Get UART pattern position
                    ...
                    {
                        uart_read_bytes(UART_NUM_1, dtmp, pos, 100 / portTICK_PERIOD_MS);            // Read UART data
                        uint8_t pat[PATTERN_CHR_NUM + 1];                                            // Variable to store UART pattern
                        memset(pat, 0, sizeof(pat));                                                 // Clear UART pattern buffer
                        uart_read_bytes(UART_NUM_1, pat, PATTERN_CHR_NUM, 100 / portTICK_PERIOD_MS); // Read UART pattern
...
void app_main(void)
{
    ...
    uart_driver_install(UART_NUM_1, BUF_SIZE * 2, BUF_SIZE * 2, 5, &uart0_queue, 0);
