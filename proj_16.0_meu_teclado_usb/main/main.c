...
static void app_send_keys(uint8_t button_state)
{
    // Send key presses based on button state
     if (button_state & (1 << 0)) {
        ...
        hid_keyboard_send((uint8_t[6]) { HID_KEY_UP } );
    }
    if (button_state & (1 << 1)) {
        ESP_LOGI(TAG, "DOWN pressed");
        hid_keyboard_send((uint8_t[6]) {HID_KEY_DOWN});
    }
    if (button_state & (1 << 2)) {
        ESP_LOGI(TAG, "LEFT pressed");
        hid_keyboard_send((uint8_t[6]) {HID_KEY_LEFT});
    }
    if (button_state & (1 << 3)) {
        ESP_LOGI(TAG, "RIGHT pressed");
        hid_keyboard_send((uint8_t[6]) {HID_KEY_RIGHT});
    }
    if (button_state & (1 << 4)) {
